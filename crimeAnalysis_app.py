...
# Optional: Show/Hide Charts
with st.expander("📌 Show Charts"):
    # 1. Bar Chart
    st.subheader("Number of Crimes per Area")
...
# 7. Regression & Extra Visuals
st.header("📈 Predictive Models and Advanced Charts")

# --- Linear Regression 1: Crime Count by Year ---
...
# --- Linear Regression 2: Victim Age over Time ---
...
# --- Linear Regression 3: Crime Count by Hour ---
...
# --- Heatmap: Area vs Month ---
st.subheader("Heatmap: Crime Frequency by Area and Month")
heat_data = filtered_df.groupby(['AREA NAME', 'Month']).size().unstack().fillna(0)
fig_heat, ax_heat = plt.subplots(figsize=(12, 6))
sns.heatmap(heat_data, cmap="YlOrBr", ax=ax_heat)
ax_heat.set_title("Crime Heatmap (Area vs Month)")
st.pyplot(fig_heat)

# --- Boxplot: Victim Age by Crime Type ---
st.subheader("Boxplot: Victim Age by Crime Type")
...
# --- Line Chart: Hourly Distribution of Crimes ---
